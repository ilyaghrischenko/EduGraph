...
def chunk_text(text: str) -> list[str]:
    # Убираем только лишние пробелы, но оставляем \n, так как они нужны сплиттеру
...
def upsert_documents(request: UpsertRequest):
    try:
        ...
        logger.error(f"Error during document upsert: {e}\n{traceback.format_exc()}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Внутрішня помилка сервера під час індексації документів."
        )
...
def delete_documents(request: DeleteRequest):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Сталася внутрішня помилка під час видалення документів."
        )
...
def search(request: SearchRequest):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Сталася внутрішня помилка під час пошуку."
        )