...
internal static class Create
{
    ...
    internal sealed class Endpoint : IEndpoint
    {
        ...
        {
            ValidationResult validateCreateAdminRequest = await validator.ValidateAsync(request, cancellationToken);

            if (!validateCreateAdminRequest.IsValid)
            {
                return Results.ValidationProblem(validateCreateAdminRequest.ToDictionary());
            }

            Result<int> createAdminResult = await handler.HandleAsync(request, cancellationToken);

            if (createAdminResult.IsFailure)
            {
                return createAdminResult.ToHttpFailure();
            }

            return TypedResults.Created($"admins/{createAdminResult.Value}", createAdminResult.Value);
        }
    }
        ...
        IPasswordHasher<User> passwordHasher) : IScopedType
    {
        public async Task<Result<int>> HandleAsync(Request request, CancellationToken cancellationToken)
        {
            ...
            IdentityResult addUserToRoleResult = await userManager.AddToRoleAsync(user, nameof(UserType.Admin));
